...
SET GROUPID=org.kendar
SET ARTIFACTID=test
SET AVERSION=1.0-SNAPSHOT
...
start /WAIT /B mvn deploy:deploy-file -e  -DgroupId=%GROUPID% -DartifactId=%ARTIFACTID% -Dversion=%AVERSION% -DgeneratePom=true -Dpackaging=jar -DrepositoryId=local -Durl=%NUROOT% -Dfile=%ARTIPATH%.jar
